import { Class7 } from 'Class7.js';
export var Class45 = function () {
    var a = {};
    a.member283 = function () {
        if (false) {
        } else {
            return '';
        }
    };
    a.member284 = function (b) {
        var c = 'precision mediump float;\n\nvarying vec2 _jf;\n\nuniform sampler2D _dc;\nuniform vec2 _di;\n\nvoid main(void) {\n\tvec4 _ou=vec4(0.0,0.0,0.0,0.0);\n\t_ou+=texture2D(_dc,_jf-_di*4.0)*0.05;\n\t_ou+=texture2D(_dc,_jf-_di*3.0)*0.09;\n\t_ou+=texture2D(_dc,_jf-_di*2.0)*0.12;\n\t_ou+=texture2D(_dc,_jf-_di)*0.15;\n\t_ou+=texture2D(_dc,_jf)*0.16;\n\t_ou+=texture2D(_dc,_jf+_di*1.0)*0.15;\n\t_ou+=texture2D(_dc,_jf+_di*2.0)*0.12;\n\t_ou+=texture2D(_dc,_jf+_di*3.0)*0.09;\n\t_ou+=texture2D(_dc,_jf+_di*4.0)*0.05;\n\tgl_FragColor=vec4(_ou.rgb,1.0);\n}';
        return c;
    };
    a.member285 = function (b) {
        var c = new Array(0);
        c.push(Class7.member189);
        c.push(Class7.member192);
        return c;
    };
    return a;
}();