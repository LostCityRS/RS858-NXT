'use strict';
import { Class1 } from 'Class1.js';
import { Class0 } from 'Class0.js';
import { Class2 } from 'Class2.js';
import { Class4 } from 'Class4.js';
import { Class5 } from 'Class5.js';
import { Class6 } from 'Class6.js';
import { Class7 } from 'Class7.js';
import { Class8 } from 'Class8.js';
import { Class9 } from 'Class9.js';
import { Class10 } from 'Class10.js';
import { Class11 } from 'Class11.js';
import { Class12 } from 'Class12.js';
import { Class13 } from 'Class13.js';
import { Class14 } from 'Class14.js';
import { Class15 } from 'Class15.js';
import { Class16 } from 'Class16.js';
import { Class17 } from 'Class17.js';
import { Class18 } from 'Class18.js';
import { Class19 } from 'Class19.js';
import { Class20 } from 'Class20.js';
import { Class21 } from 'Class21.js';
import { Class22 } from 'Class22.js';
import { Class23 } from 'Class23.js';
import { Class24 } from 'Class24.js';
import { Class25 } from 'Class25.js';
import { Class26 } from 'Class26.js';
import { Class27 } from 'Class27.js';
import { Class28 } from 'Class28.js';
import { Class29 } from 'Class29.js';
import { Class30 } from 'Class30.js';
import { Class31 } from 'Class31.js';
import { Class32 } from 'Class32.js';
import { Class33 } from 'Class33.js';
import { Class34 } from 'Class34.js';
import { Class35 } from 'Class35.js';
import { Class36 } from 'Class36.js';
import { Class37 } from 'Class37.js';
import { Class38 } from 'Class38.js';
import { Class39 } from 'Class39.js';
import { Class40 } from 'Class40.js';
import { Class41 } from 'Class41.js';
import { Class42 } from 'Class42.js';
import { Class43 } from 'Class43.js';
import { Class44 } from 'Class44.js';
import { Class45 } from 'Class45.js';
import { Class46 } from 'Class46.js';
import { Class47 } from 'Class47.js';
import { Class48 } from 'Class48.js';
import { Class49 } from 'Class49.js';
import { Class50 } from 'Class50.js';
import { Class51 } from 'Class51.js';
import { Class52 } from 'Class52.js';
import { Class53 } from 'Class53.js';
import { Class54 } from 'Class54.js';
import { Class55 } from 'Class55.js';
import { Class56 } from 'Class56.js';
import { Class57 } from 'Class57.js';
import { Class58 } from 'Class58.js';
import { Class59 } from 'Class59.js';
import { Class60 } from 'Class60.js';
import { Class61 } from 'Class61.js';
import { Class62 } from 'Class62.js';
import { Class63 } from 'Class63.js';
import { Class64 } from 'Class64.js';
import { Class65 } from 'Class65.js';
import { Class66 } from 'Class66.js';
import { Class67 } from 'Class67.js';
import { Class68 } from 'Class68.js';
import { Class69 } from 'Class69.js';
import { Class70 } from 'Class70.js';
import { Class71 } from 'Class71.js';
import { Class72 } from 'Class72.js';
import { Class73 } from 'Class73.js';
import { Class74 } from 'Class74.js';
import { Class75 } from 'Class75.js';
import { Class76 } from 'Class76.js';
import { Class77 } from 'Class77.js';
import { Class78 } from 'Class78.js';
import { Class79 } from 'Class79.js';
import { Class80 } from 'Class80.js';
import { Class81 } from 'Class81.js';
import { Class82 } from 'Class82.js';
import { Class83 } from 'Class83.js';
import { Class84 } from 'Class84.js';
import { Class85 } from 'Class85.js';
import { Class86 } from 'Class86.js';
import { Class87 } from 'Class87.js';
import { Class88 } from 'Class88.js';
import { Class89 } from 'Class89.js';
import { Class90 } from 'Class90.js';
import { Class91 } from 'Class91.js';
import { Class92 } from 'Class92.js';
import { Class93 } from 'Class93.js';
import { Class94 } from 'Class94.js';
import { Class95 } from 'Class95.js';
import { Priority } from 'Class96.js';
import { Class97 } from 'Class97.js';
import { Class98 } from 'Class98.js';
import { Class99 } from 'Class99.js';
import { Class100 } from 'Class100.js';
import { Class101 } from 'Class101.js';
import { Class102 } from 'Class102.js';
import { Class103 } from 'Class103.js';
import { Class104 } from 'Class104.js';
import { Class105 } from 'Class105.js';
import { Class106 } from 'Class106.js';
import { Class107 } from 'Class107.js';
import { Class108 } from 'Class108.js';
import { Class109 } from 'Class109.js';
import { Class110 } from 'Class110.js';
import { Class111 } from 'Class111.js';
import { Class112 } from 'Class112.js';
import { Class113 } from 'Class113.js';
import { Class114 } from 'Class114.js';
import { Class115 } from 'Class115.js';
import { Class116 } from 'Class116.js';
import { Class117 } from 'Class117.js';
import { Class118 } from 'Class118.js';
import { Class119 } from 'Class119.js';
import { Class120 } from 'Class120.js';
import { Class121 } from 'Class121.js';
import { CP1252 } from 'Class122.js';
import { Class123 } from 'Class123.js';
import { Packet } from 'Class124.js';
import { Class125 } from 'Class125.js';
import { Class126 } from 'Class126.js';
import { Class127 } from 'Class127.js';
import { Class128 } from 'Class128.js';
import { Class129 } from 'Class129.js';
import { Class130 } from 'Class130.js';
import { Js5ConfigGroup } from 'Class131.js';
import { Class132 } from 'Class132.js';
import { Class133 } from 'Class133.js';
import { Class134 } from 'Class134.js';
import { Class135 } from 'Class135.js';
import { Class136 } from 'Class136.js';
import { Class137 } from 'Class137.js';
import { Class138 } from 'Class138.js';
import { Class139 } from 'Class139.js';
import { Class140 } from 'Class140.js';
import { Class141 } from 'Class141.js';
import { Class142 } from 'Class142.js';
import { Class143 } from 'Class143.js';
import { Class144 } from 'Class144.js';
import { Class145 } from 'Class145.js';
import { Class146 } from 'Class146.js';
import { Class147 } from 'Class147.js';
import { Class148 } from 'Class148.js';
import { Class149 } from 'Class149.js';
import { Class150 } from 'Class150.js';
import { Class151 } from 'Class151.js';
import { Class152 } from 'Class152.js';
import { Class153 } from 'Class153.js';
import { Class154 } from 'Class154.js';
import { Class155 } from 'Class155.js';
import { Class156 } from 'Class156.js';
import { Class157 } from 'Class157.js';
import { Class158 } from 'Class158.js';
import { Class159 } from 'Class159.js';
import { Class160 } from 'Class160.js';
import { Class161 } from 'Class161.js';
import { Class162 } from 'Class162.js';
import { Class163 } from 'Class163.js';
import { Class164 } from 'Class164.js';
import { Class165 } from 'Class165.js';
import { Cuboid } from 'Class166.js';
import { Class167 } from 'Class167.js';
import { Class168 } from 'Class168.js';
import { Class169 } from 'Class169.js';
import { HillChange } from 'Class170.js';
import { Class171 } from 'Class171.js';
import { Class172 } from 'Class172.js';
import { Class173 } from 'Class173.js';
import { Class174 } from 'Class174.js';
import { Class175 } from 'Class175.js';
import { Class176 } from 'Class176.js';
import { Class177 } from 'Class177.js';
import { Class178 } from 'Class178.js';
import { Class179 } from 'Class179.js';
import { Class180 } from 'Class180.js';
import { Class181 } from 'Class181.js';
import { Class182 } from 'Class182.js';
import { Class183 } from 'Class183.js';
import { Class184 } from 'Class184.js';
import { Class185 } from 'Class185.js';
import { Class186 } from 'Class186.js';
import { Class187 } from 'Class187.js';
import { Class188 } from 'Class188.js';
import { Class189 } from 'Class189.js';
import { Class190 } from 'Class190.js';
import { Class191 } from 'Class191.js';
import { Class192 } from 'Class192.js';
import { Class193 } from 'Class193.js';
import { Class194 } from 'Class194.js';
import { Class195 } from 'Class195.js';
import { Class196 } from 'Class196.js';
import { Class197 } from 'Class197.js';
import { Class198 } from 'Class198.js';
import { Class199 } from 'Class199.js';
import { Class200 } from 'Class200.js';
var console = {};
var run = function () {
    member53.member25(10 * 1024 * 1024);
    var O = 0;
    var z = 1;
    var s = 2;
    var J = {};
    var c = false;
    var t;
    var g = 0;
    var Q = [];
    var q;
    var H;
    var o = 0, p = 0;
    var r = 0;
    var e = 0;
    var v = 0;
    var h = 0;
    var B = 0;
    var K = 0;
    var A = [];
    self.postMessage = member55(self, 'postMessage');
    var L = {
        member59: Class197.LOG,
        logMessage: null
    };
    console.log = function (R) {
        L.logMessage = R;
        self.postMessage(L);
    };
    var I = {
        member59: Class197.ERROR,
        stack: null,
        member2375: -1
    };
    console.error = function (S, R) {
        I.stack = S.stack;
        I.member2375 = R;
        self.postMessage(I);
    };
    var N = -1;
    var d = -1;
    var B = 0;
    var m = 0;
    var a = function () {
        if (g === 0 && N !== -1) {
            B += Date.now() - N;
            N = -1;
        } else if (g > 0) {
            d = Date.now();
        } else {
            throw new Error('1800 ' + g);
        }
        clearInterval(q);
        q = undefined;
    };
    var D = function () {
        if (g > 0 && d !== -1) {
            m += Date.now() - d;
            d = -1;
        }
        if (g > 0 && N === -1) {
            N = Date.now();
        }
        if (g < 0) {
            throw new Error('1801 ' + g);
        }
        if (q !== undefined) {
            return;
        }
        q = setInterval(b, 5);
    };
    var E = function (R) {
        var S = Q[R];
        return 'Worker ' + t + ': ' + S.name + ' ' + S.status;
    };
    var l = function (R, S) {
        var T = Q[R];
        if (false) {
        }
        T.status = S;
        if (false) {
        }
    };
    self.onmessage = function (R) {
        var Y = false;
        if (false) {
        }
        var S = R.data;
        for (var X = 0; X < S.length; X++) {
            var V = S[X];
            var T = Date.now();
            var U = V.member59;
            if (U === Class197.member2312) {
                P(V.member2376);
            } else if (U === Class197.member2315) {
                Class147.member25(V.member2377);
                Class154.member25({ member1546: V.member1546 });
            } else if (U === Class197.member2318) {
                Class174.member25(V.member2378);
            } else if (U === Class197.member2321) {
                Class185.member25(V.member2379);
            } else if (U === Class197.member2322) {
                Class137.member25(V.member2380);
            } else if (U === Class197.member2317) {
                self.gc();
            } else if (U === Class197.member2319) {
                console.log(requireJS_logConstructionCounts());
            } else if (U === Class197.member2320) {
                if (false) {
                }
            } else if (U === Class197.member2314) {
                var W = Q[V.member2375];
                l(V.member2375, O);
                W.member1646(J, V.member2381, V.member2382);
                if (!M(V.member2375) && Q[V.member2375].status === O) {
                    D();
                }
            } else if (U === Class197.member2316) {
                var W = Q[V.member2375];
                l(V.member2375, O);
                var Z = Class151(V.member1569);
                W.member1648(Z, V.member2381);
                if (!M(V.member2375) && Q[V.member2375].status === O) {
                    D();
                }
            } else if (U === Class197.member2328) {
                k(Class199({ request: V.request }), V.member2375);
            } else if (U === Class197.member2326) {
                k(Class159({ request: V.request }), V.member2375);
            } else if (U === Class197.member2325) {
                k(Class193({ request: V.request }), V.member2375);
            } else if (U === Class197.member2327) {
                k(Class200({ request: V.request }), V.member2375);
            } else if (U === Class197.member2329) {
                k(Class196({ request: V.request }), V.member2375);
            } else {
                throw new Error('1802 ' + U);
            }
            e += Date.now() - T;
        }
        if (false) {
        }
    };
    var k = function (S, R) {
        g++;
        D();
        if (Q[R] === undefined) {
            Q[R] = S;
            S.member2375 = R;
            l(R, O);
            M(R);
        } else {
            throw new Error('1803 ');
        }
    };
    var P = function (R) {
        if (false) {
        }
        t = R;
        G.member2376 = t;
        x.member2376 = t;
        F.member2376 = t;
        y.member2376 = t;
        r = Date.now();
        H = r;
        h = Math.floor(r / 1000);
    };
    var G = {
        member59: Class197.COMPLETE,
        member2376: -1,
        member2375: -1,
        result: undefined
    };
    var M = function (S) {
        var U = Q[S];
        if (U === undefined) {
            return true;
        }
        if (U.status !== O) {
            return true;
        }
        var T = U.name;
        if (false) {
        }
        var R = U.member78(J, S);
        if (false) {
        }
        if (R === null) {
            return false;
        }
        if (false) {
        }
        G.member2375 = U.member2375;
        G.result = R.member1658;
        self.postMessage(G, R.member1659);
        if (false) {
        }
        if (U.tidyup !== undefined) {
            U.tidyup();
        }
        Q[U.member2375] = undefined;
        G.result = undefined;
        if (g === 0) {
            throw new Error('1804 ');
        }
        g--;
        return true;
    };
    var x = {
        member59: Class197.member2313,
        member2376: -1,
        member2376: -1,
        activity: -1
    };
    var b = function () {
        if (false) {
        }
        try {
            var S = Date.now();
            if (H < S - 1000) {
                o = Math.floor(p * 1000 / (S - H));
                p = 0;
                H = S;
                if (h < Math.floor(S / 1000) - 10) {
                    r = S;
                    e = 0;
                    v = 0;
                    h = Math.floor(S / 1000);
                }
                var W = v + e;
                var V = Math.floor(W / (Date.now() - r) * 100);
                x.member2376 = o;
                x.activity = V;
                self.postMessage(x);
            }
            var T = true;
            for (var R = 0; R < Q.length; R++) {
                T = M(R) && T;
            }
            if (T) {
                a();
            }
            v += Date.now() - S;
        } catch (U) {
            console.error(U);
        }
        if (false) {
        }
    };
    J.member78 = b;
    var F = {
        member59: Class197.member2314,
        member2376: -1,
        member2383: -1,
        member2384: undefined,
        member2381: -1,
        member2375: -1
    };
    var u = [0];
    var n = function (U, S, R, T) {
        F.member2383 = U;
        u[0] = S;
        F.member2384 = u;
        F.member2381 = T;
        F.member2375 = R;
        postMessage(F);
        l(R, z);
        F.member2384 = undefined;
    };
    J.member2385 = n;
    var j = function (U, R, S, T) {
        F.member2383 = U;
        F.member2384 = R;
        F.member2381 = T;
        F.member2375 = S;
        postMessage(F);
        l(S, z);
        F.member2384 = undefined;
    };
    J.member1652 = j;
    var y = {
        member59: Class197.member2316,
        member2376: -1,
        member2375: -1,
        materialIDList: undefined,
        member2381: -1
    };
    var C = function (R, S, T) {
        y.member2375 = S;
        y.materialIDList = R;
        y.member2381 = T;
        postMessage(y);
        l(S, s);
        y.materialIDList = undefined;
    };
    J.member1654 = C;
    var i = function () {
        return t;
    };
    J.member1645 = i;
    return J;
}();